...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
    <div
        ...
        <p
            class="text-sm font-medium {{ $stats['patients_growth'] >= 0 ? 'text-success' : 'text-danger' }} flex items-center gap-1">
            <span class="material-symbols-outlined text-base">
                {{ $stats['patients_growth'] >= 0 ? 'arrow_upward' : 'arrow_downward' }}
            </span>
            {{ $stats['patients_growth'] >= 0 ? '+' : '' }}{{ number_format($stats['patients_growth'], 1) }}%
        </p>
    </div>
    ...
    <div
        ...
        <p class="text-4xl font-bold text-body-text dark:text-body-text-dark">
            {{ number_format($stats['active_pharmacies']) }}</p>
        <p
            class="text-sm font-medium {{ $stats['pharmacies_growth'] >= 0 ? 'text-success' : 'text-danger' }} flex items-center gap-1">
            <span class="material-symbols-outlined text-base">
                {{ $stats['pharmacies_growth'] >= 0 ? 'arrow_upward' : 'arrow_downward' }}
            </span>
            {{ $stats['pharmacies_growth'] >= 0 ? '+' : '' }}{{ number_format($stats['pharmacies_growth'], 1) }}%
        </p>
    </div>
    ...
    <div
        ...
        <p class="text-4xl font-bold text-body-text dark:text-body-text-dark">
            {{ number_format($stats['prescriptions_today']) }}</p>
        <p
            class="text-sm font-medium {{ $stats['prescriptions_growth'] >= 0 ? 'text-success' : 'text-danger' }} flex items-center gap-1">
            <span class="material-symbols-outlined text-base">
                {{ $stats['prescriptions_growth'] >= 0 ? 'arrow_upward' : 'arrow_downward' }}
            </span>
            {{ $stats['prescriptions_growth'] >= 0 ? '+' : '' }}{{ number_format($stats['prescriptions_growth'], 1) }}%
        </p>
    </div>
    ...
    <div
        ...
        <p class="text-4xl font-bold text-body-text dark:text-body-text-dark">
            {{ number_format($stats['avg_fulfillment_time'], 1) }}h</p>
        <p
            class="text-sm font-medium {{ $stats['fulfillment_trend'] <= 0 ? 'text-success' : 'text-danger' }} flex items-center gap-1">
            <span class="material-symbols-outlined text-base">
                {{ $stats['fulfillment_trend'] <= 0 ? 'arrow_downward' : 'arrow_upward' }}
            </span>
            {{ $stats['fulfillment_trend'] <= 0 ? '' : '+' }}{{ number_format($stats['fulfillment_trend'], 1) }}%
        </p>
    </div>
</div>
        ...
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-baseline gap-2">
                <p class="text-2xl font-bold text-body-text dark:text-body-text-dark">
                    {{ number_format($registrations['total']) }}</p>
                <p class="text-sm font-medium {{ $registrations['growth'] >= 0 ? 'text-success' : 'text-danger' }}">
                    {{ $registrations['growth'] >= 0 ? '+' : '' }}{{ number_format($registrations['growth'], 1) }}%
                </p>
            </div>
        </div>
        <div
    ...
    <div class="flex flex-col gap-4">
        <div
            ...
            <div class="flex flex-col gap-2">
                <a href="{{ route('admin.users') }}" data-spa
